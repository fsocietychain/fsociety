...
*Setup instructions for a Gitian build of Fsociety Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fsociety
...
- [Building Fsociety Core](#building-fsociety-core)
...
Clone the git repositories for Fsociety Core and Gitian.
...
in the Fsociety Core repository under 'Fetch and create inputs' to install sources which require
...
To build Fsociety Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Fsociety Core repository.
