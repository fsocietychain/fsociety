Cross-compiliation of Fsociety Core
===============================

Fsociety Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Fsociety.
...
When building Fsociety Core, use
...
Note that for WSL the Fsociety Core source path MUST be somewhere in the default mount file system, for
...
It's not possible to build the Fsociety Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Fsociety Core source code).
