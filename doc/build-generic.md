GENERIC BUILD NOTES
====================
Some notes on how to build Fsociety Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build Fsociety with system-wide installed dependencies
...
------------------------------------
Building the dependencies and Fsociety Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
Fsociety inherited the `depends` folder from Bitcoin, which contains all dependencies required to build Fsociety. These
dependencies must be built before Fsociety can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build Fsociety Core. Caching of build results will ensure that only
...
not done, build failures might occur when building Fsociety.
...
ccache
------
`./configure` of Fsociety Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
