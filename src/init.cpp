...
public:
    ...
    // Fsociety Specific WalletInitInterface InitCoinJoinSettings
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-platform-user=<user>", "Set the username for the \"platform user\", a restricted user intended to be used by Fsociety Platform, to the specified username.", false, OptionsCategory::SMARTNODE);
...
/** Sanity checks
 *  Ensure that Fsociety Core is running in a usable environment with all
...
{
    // Make sure only a single Fsociety Core process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Fsociety Core is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Fsociety Core is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 10c: schedule Fsociety-specific tasks
